...
nodes = read.csv("cleaned data/nodes-2020-02-21.csv", header = TRUE, as.is=TRUE)
edges = read.csv("cleaned data/edges-2020-02-21.csv", header = TRUE, as.is=TRUE)
names(nodes)[1] = "ID"
m=as.matrix(edges)
...
# of specific first-generation articles: XX
# other first-generation articles (citing): XX
# of second-generation articles: XX
...
# Jodi's presentation #
#######################
# The first-generation network by itself, year by year (or if that’s hard - just 2008 and 2019)
dynamic = function(y) {
  freq_table = dynamicnodes %>% 
    filter(Year <= y) %>% 
    group_by(group = substr(ID, 1, 4)) %>% 
  ...
  n = dynamicnodes[which((nchar(as.character(dynamicnodes$ID)) <= 4) & dynamicnodes$Year <= y),]
  e = edges[((edges$from %in% n$ID) & (edges$to %in% n$ID)),]
  net = graph_from_data_frame(e, n, directed = T)
  V(net)$color = ifelse((nchar(as.character(V(net)$name)) <= 4), 
                        ifelse(V(net)$name == "A000", "black", "lightblue"), "mediumpurple")
...
# The WHOLE second-generation network, year by year (or if that’s hard - just 2008 and 2019)
dynamic = function(y) {
  freq_table = dynamicnodes %>% 
    filter(Year <= y) %>% 
    group_by(group = substr(ID, 1, 4)) %>% 
